  ...
  for(iter in seq(start, maxiter, 1)) # cseiler: in case of interruption, continue with last iterration
     {
      ...
      if(!parallel)
        ...
        # Create the farm
        farmName <- "CLASSIC_fit"
        if (file.exists(farmName)) {system(sprintf("rm -rf %s", farmName))}
        Sys.sleep(1)
        ...
        indivsFile <- paste0("/home/aadams/projects/def-cseiler-ab/aadams/data-assimilation/", farmName, "/", indivsFile)
        valFile <- paste0("/home/aadams/projects/def-cseiler-ab/aadams/data-assimilation/", farmName, "/", argValsFile)
        typeFile <- paste0("/home/aadams/projects/def-cseiler-ab/aadams/data-assimilation/", farmName, "/", argTypesFile)
        lenFile <- paste0("/home/aadams/projects/def-cseiler-ab/aadams/data-assimilation/", farmName, "/", argLensFile)
        nameFile <- paste0("/home/aadams/projects/def-cseiler-ab/aadams/data-assimilation/", farmName, "/", argNamesFile)
        
        for (id in 1:popSize) {
          sourceCostFunc <- "R -e \"source(\'/home/aadams/daisy/daisy/R/cost.fun.R\')"
          callCostFunc <- paste0("costFunc('", id, "', '", indivsFile, "', '", valFile, "', '",
                                typeFile, "', '", lenFile, "', '", nameFile, "', '",
                                farmName, "')")
          #callCostFunc <- paste0("costFunc ", id, " ", indivsFile, " ", valFile, " ",
          #                       typeFile, " ", lenFile, " ", nameFile)
          line <- paste0(sourceCostFunc, "; ", callCostFunc, "\"")
