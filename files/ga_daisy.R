               ...
               seed = NULL) 
{
  ...
  object@population <- Pop
      ...
      start <- 1 # cseiler: start of iterration
  ...
  for(iter in seq(start, maxiter, 1)) # cseiler: in case of interruption, continue with last iterration
     {
      ...
      if(!parallel)
        ...
        # Set the names of all the text files to store argument information.
        ...
        for (arg in callArgs) {
          invisible(cat(as.character(arg), "\n", file = argValsFile, append = TRUE))
        ...
        for (indiv in getFitFor) {
          invisible(cat(unlist(indiv), "\n", file = indivsFile, append = TRUE))
        }
        
        # Create the table.dat file
        dataAssimFolder <- "/home/aadams/projects/def-cseiler-ab/aadams/data-assimilation"
        indivsFile <- paste0(dataAssimFolder, "/", farmName, "/", indivsFile)
        # Each case has its own val file that will be created in setArgs.R.
        valFile <- paste0(dataAssimFolder, "/", farmName)
        typeFile <- paste0(dataAssimFolder, "/", farmName, "/", argTypesFile)
        lenFile <- paste0(dataAssimFolder, "/", farmName, "/", argLensFile)
        nameFile <- paste0(dataAssimFolder, "/", farmName, "/", argNamesFile)
        
        argsToAdjust <- list("parameterFile, run_classic_file")
        system("chmod +x /home/aadams/projects/def-cseiler-ab/aadams/data-assimilation/setArgs.R")
        ...
        for (id in 1:popSize) {
          ...
          sourceSetArgs <- "source('/home/aadams/projects/def-cseiler-ab/aadams/data-assimilation/setArgs.R')"
          callSetArgs <- paste0("setArgs('", id, "', '", dataAssimFolder, "', '",
                                nameFile, "', '", valFile, "/argValues.txt", "', '", 
                                argsToAdjust, "', '", farmName, "')")
          callCostFunc <- paste0("cost.fun('", id, "', '", indivsFile, "', '", 
                                 valFile, "/RUN", id, "/argValues.txt", "', '", 
                                 typeFile, "', '", lenFile, "', '", nameFile, "', '",
                                 dataAssimFolder, "')")
          line <- paste0(appendLibPaths, "; ", loadDaisy, "; ", sourceSetArgs, "; ",
                         callSetArgs, "; ", callCostFunc, "\"")
      ...
      # Get the fitness values from all the individuals.
      
      # Wait until the simInfo file for the last run exists, then read in all the fitness values.
      Sys.sleep(2)
      lastInfoFile <- paste0("RUN", popSize)
      while (!file.exists(paste0(lastInfoFile, "/simInfo.txt"))) {
        Sys.sleep(5)
      }
      
      Sys.sleep(5)
      
      # Read in all the fitness values, located on line 2 of the simInfo.txt file of each run
      
      library(readr)
      
      for (instance in 1:popSize) {
        f <- paste0("RUN", instance, "/simInfo.txt")
        score <- read_lines(f, skip = 1, n_max = 1)
        Fitness[instance] <- score
      }
      
      # Move all RUNX folders to the GEN folder
      gen <- 1
      genFolder <- paste0("GEN", gen)
      while (file.exists(genFolder) && gen < maxiter) {
        gen <- gen + 1
        genFolder <- paste0("GEN", gen)
      }
      ...
      # if(iter == 81) {print(Pop)} # cseiler not being called
      # if(iter == 81) {print(Fitness)} # cseiler not being called
    ...
    #  if(iter == 81) {print(Pop)} # cseiler 
    #  if(iter == 81) {print(Fitness)} # cseiler
        }
      else
      ...
      # if(iter == 81) {print(Pop)} # cseiler Pop differs 
      # if(iter == 81) {print(Fitness)} # cseiler Fitness differs
