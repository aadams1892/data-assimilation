...
#' @param argValsFile File containing the values for all the cost function parameters
...
#' @param dataAssimFolder The file path to the data-assimilation folder
#' @export

cost.fun <- function(identifier, indivFile, argValsFile, argTypesFile, 
                     argLengthsFile, argNamesFile, dataAssimFolder) {
    ...
    Sys.sleep(2)
    # Move the finished run into the simulations folder.
    simulationsFolder <- paste0(dataAssimFolder, "/simulations")
    moveModelOutputFolder <- paste("mv", NewModelOutputFolder, simulationsFolder)
    system(moveModelOutputFolder)
